...
function outputZodProperty(property: Property, resourceType: string): string {
  ...
  if (property.property === 'resourceType') {
    return `resourceType: z.literal('${resourceType}')`;
  }
...
function collectSchemaFunctions(prop: Property): Set<string> {
  ...
  if (prop.type === 'integer64') funcs.add('createInteger64Schema');
...
(async () => {
  const baseOutputDir = path.join(__dirname, '..', '..', '..', 'src', 'v5.0.0');
      ...
      ${group.types
        .map((type) => {
          ...
          return schema.replace(
            /create(Base64Binary|Boolean|Canonical|Code|Date|DateTime|Decimal|Id|Instant|Integer|Integer64|Markdown|Oid|PositiveInt|String|Time|UnsignedInt|Uri|Url|Uuid|Xhtml)Schema\(\)/g,
